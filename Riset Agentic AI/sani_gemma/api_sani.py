...
def _clean_chat_output(raw: str) -> str:
    ...
    - blok [ACTION], [RESPONSE], dsb
    - baris INTENT:, ACTION:, FOTO_MENU:, BOT_INTEGRATION:
    - code block ```...``` dan potongan JSON di bawahnya
    """
    if not raw:
        return ""
    ...
    # 1) buang blok [ACTION] ... [/ACTION] / [RESPONSE] ...
    text = re.sub(r"\[ACTION\].*?\[/ACTION\]", "", text, flags=re.DOTALL)
    text = re.sub(r"\[RESPONSE\].*?\[/RESPONSE\]", "", text, flags=re.DOTALL)

    # 2) buang semua tag [HURUF_KAPITAL_123]
    text = re.sub(r"\[[A-Z0-9_]+\]", "", text)

    # 3) buang baris yang mengandung INTENT:, ACTION:, FOTO_MENU:, BOT_INTEGRATION:
    text = re.sub(r"^.*INTENT:.*$", "", text, flags=re.MULTILINE)
    text = re.sub(r"^.*ACTION:.*$", "", text, flags=re.MULTILINE)
    text = re.sub(r"^.*FOTO_MENU:.*$", "", text, flags=re.MULTILINE)
    text = re.sub(r"^.*BOT_INTEGRATION:.*$", "", text, flags=re.MULTILINE)

    # 4) buang code-block dan JSON/struktur menu setelah teks utama
    text = text.split("```")[0]
    text = re.split(r"\n\s*[-–—]{3,}", text)[0]
    text = re.split(r"\n\s*\{", text)[0]

    # Jika masih full JSON (mulai { di awal & } di akhir), buang saja
    tstrip = text.strip()
    if tstrip.startswith("{") and tstrip.endswith("}"):
        return ""

    # 5) rapikan spasi & baris kosong berlebih
